...
const UpdateProduct = () => {
  ...
  const handlePostProduct = async (data) => {
    setLoading(true);
    const newImage = await imagesUpload(filesValue);
    data.images = [...newImage, ...images];
    console.log(data);
    try {
      const result = await axiosPublic.patch(`/product/${_id}/update`, data);
      console.log(result);
        ...
        <form onSubmit={handleSubmit(handlePostProduct)}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="boxStyle order-4 sm:order-2">
              ...
              {/* preview */}
              <div
                className={`h-42 bg-green-200/20 border border-dashed rounded-lg border-primary p-4 flex items-center justify-center gap-2`}
              >
                {previewImg.length === 0
                  ? "Preview Images"
