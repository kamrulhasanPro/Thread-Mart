...
const ManageAllProducts = () => {
  ...
  const {
    ...
    queryKey: ["manage-all-products", user?.email],
    queryFn: async () => (await axiosPublic(`/manage-all-products`)).data,
    enabled: !!user?.email,
  });
  ...
  const handleShowOnHomeEdit = async (value, id) => {
    console.log(value, id);
    const result = await axiosPublic.patch(`/product/${id}/update`, {
      showOnHomePage: value,
    });
    if (result.data.modifiedCount) {
      queryClient.invalidateQueries("our-products");
      toast.success(`${value ? "Add" : "Remove"} this product in Home Page`);
    }
  };
      ...
      <div>
        <div className="overflow-x-auto border border-gray-600/50 rounded-xl max-h-[calc(100vh-100px)]">
          <table className="table text-nowrap">
            {/* head */}
            <thead className="sticky top-0 bg-secondary z-50">
              <tr>
                <th></th>
                <th>Product</th>
                <th>Price</th>
                ...
                <tr key={product._id}>
                  ...
                  <td>
                    <input
                      type="checkbox"
                      onChange={(e) =>
                        handleShowOnHomeEdit(e.target.checked, product._id)
                      }
